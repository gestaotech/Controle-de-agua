          ...
          <tbody>
            {leituras.map(l => (
              <tr key={l.id}>
                ...
                <td>{l.mes}</td>
                <td>{l.consumo} m³</td>
                <td><button onClick={() => gerarFatura(l)} disabled={gerando} style={{ padding: '4px 12px', background: gerando ? '#94A3B8' : '#3B82F6', color: '#fff', border: 'none', borderRadius: 6, cursor: gerando ? 'not-allowed' : 'pointer', fontSize: '0.85rem' }}>{gerando ? 'Gerando...' : 'Gerar Fatura'}</button></td>
              </tr>
            ))}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 16 }} onClick={e => { if (e.target === e.currentTarget) fecharFatura(); }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 32, maxWidth: 580, width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
              ...
              <div><label style={{ fontSize: '0.75rem', color: '#64748B', textTransform: 'uppercase' as const }}>Bairro</label><div>{fatura.unidade?.bairros?.nome}</div></div>
            ...
            {fatura.pixErro && (
              <div style={{ background: '#FEF3C7', border: '1px solid #FCD34D', borderRadius: 8, padding: 12, marginBottom: 16 }}>
                ...
                <p style={{ color: '#92400E', fontSize: '0.8rem', margin: '4px 0 0 0' }}>Verifique se a chave API do Asaas esta configurada corretamente.</p>
              </div>
            )}
